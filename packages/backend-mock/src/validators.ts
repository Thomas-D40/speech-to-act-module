import { IntentionContract, DOMAINS, ValidationError } from './types';

export function validateIntentionContract(contract: any): {
  isValid: boolean;
  ...
  } else if (!Object.values(DOMAINS).includes(contract.domain)) {
    errors.push({
      field: 'domain',
      message: `Invalid domain. Must be one of: ${Object.values(DOMAINS).join(', ')}`,
      code: 'INVALID_DOMAIN'
    });
  }
