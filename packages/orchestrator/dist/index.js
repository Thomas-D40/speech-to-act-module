...
import { DeterministicMapper, } from '@speech-to-act/deterministic-mapping';
...
app.post('/api/process', async (req, res) => {
    const { facts, targets, workflow = 'safe' } = req.body;
    // Validate input
 ...
 */
app.post('/api/process/confirm', async (req, res) => {
 ...
 */
app.post('/api/process/reject', async (req, res) => {
    const { pending_id } = req.body;
    if (!pending_id) {
        return res.status(400).json({
            success: false,
