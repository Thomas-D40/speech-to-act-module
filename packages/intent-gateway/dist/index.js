 ...
 */
app.post('/api/intents/preview', async (req, res) => {
 ...
 */
app.get('/api/intents/pending', (_, res) => {
 ...
 */
app.post('/api/intents/confirm', async (req, res) => {
 ...
 */
app.post('/api/intents/reject', (req, res) => {
    const { pending_id } = req.body;
    if (!pending_id) {
        return res.status(400).json({
            success: false,
 ...
 */
app.post('/api/intents/commit', async (req, res) => {
    const contract = req.body;
    // Local validation
    const validation = validateIntentionContract(contract);
    if (!validation.isValid) {
